...
| Audit (implicit) | `CreatedAt`, `CreatedBy`, `ModifiedAt`, `ModifiedBy`, `IsActive` | Audit metadata inherited by most aggregates. | Present on AuditableEntity descendants; no centralized log. 【F:backend/Payroll.Domain/Common/AuditableEntity.cs†L1-L23】|
